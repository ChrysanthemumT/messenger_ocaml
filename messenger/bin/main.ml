...
module Server = struct
  ...
  let handle_conn ic oc = Lwt.join [ send_message oc (); receive_message ic oc ]
  ...
  let accept_conn conn =
    ...
    Lwt.on_failure (handle_conn ic oc) (fun e -> ignore e);
    return ()

  let start_server sock =
    let rec serve () = Lwt_unix.accept sock >>= accept_conn >>= serve in
    serve
end
...
module Client = struct
   ...
   fun () ->
    Lwt.catch
      (fun () ->
        ...
        let send_time = Unix.gettimeofday () in
        let encoded = Printf.sprintf "%f:%s" send_time message in
        write_message encoded oc >>= send_message oc)
      (function
        | End_of_file -> Lwt.return_unit
        | exn ->
  ...
  let handle_conn oc ic = Lwt.join [ send_message oc (); receive_message ic oc ]
  ...
  let start_client server_addr port =
    ...
    let rec loop () = handle_conn oc ic >>= fun () -> loop () in
    loop ()
end
